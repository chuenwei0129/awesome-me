...
在上面添加光标：`option + command + up`
在下面添加光标：`option + command + xia`
8↓
在行尾添加光标：
に合
添加下一个匹配项：
...
添加上一个匹配项 选择所有匹配项：
合8
切换为“Cmd+单击”进行多光标功能 列选择模式

, select all occurrences, add additional cursors and more intellisense-get code assistance and parameter suggestions for your code and external modules ine Actions-quickly move lines around to re-order your code
