...
class NativeUnit extends Unit {
  ...
  diff(diffQueue, nextChildReactElements) {
    const prevChildReactUnitsMap = this.getPrevChildReactUnitsMap(
      this._childReactUnits
    )
    const {
      nextChildReactUnits,
      nextChildReactUnitsMap
    } = this.getNextChildReactUnits(
    ...
    for (const [key, val] of Object.entries(nextProps)) {
      if (key === 'children') {
        ...
        Object.entries(val).forEach(([k, v]) => {
          document.querySelector(`[data-reactid="${this._reactid}"]`).style[
            k
          ] = v
        })
        ...
        // class
        document
          .querySelector(`[data-reactid="${this._reactid}"]`)
...
class ComponentUnit extends Unit {
  ...
  update(nextReactElement, nextState) {
    ...
    if (
      shouldDeepCompare(
        this._renderedReactUnit._currentReactElement,
      ...
      document.querySelector(
        `[data-reactid="${this._reactid}"]`
      ).innerHTML = createReactUnit(this._nextReactElement).create(
        this._reactid
      )
    }
...
export function createReactUnit(reactElement) {
  ...
  if (
    reactElement instanceof Element &&
