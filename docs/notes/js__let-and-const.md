...
**使用 `let` 时，JavaScript 引擎内部会记住上一轮循环的值，初始化本轮的变量 `i` 时，就在上一轮循环的基础上进行计算。**

### [循环变量的作用域](https://twitter.com/NicoloRibaudo/status/1591427882509045760)

This for loop:

```js
// `for` 循环设置循环变量的那部分是一个父作用域，所以闭包捕获的变量是 i === 0。
...
同一段代码为了能够在各种环境都能取到顶层对象，现在一般是使用 `this` 变量，但是有局限性。
