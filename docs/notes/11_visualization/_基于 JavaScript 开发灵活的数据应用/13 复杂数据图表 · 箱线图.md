...
箱线图是利用离散数据中的 5 个统计量进行绘制的：最小值、第一四分位数 $Q_1$、中位数、第三四分位数 $Q_3$ 以及最大值，并利用 $Q_1$ 和 $Q_3$ 两个四分位数计算得到四分位距 $IQR$。
...
* 如果离散数据的个数 $n$ 为奇数，则中位数就为第 $\frac{n+1}{2}$ 个数值，$median=S_{\frac{n+1}{2}}$；
* 若数据的个数 $n$ 为偶数，则中位数为最中间两个数值的平均值，$median=\frac{1}{2}(S_{\frac{n}{2}}+S_{\frac{n}{2}+1})$。
...
![四分位数](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/8/12/1652bb63557e8b98~tplv-t2oaga2asx-image.image)

当中位数的位置上不存在某一个特定的数值时，则取最中间两个数值的平均数。而第一和第三四分位数则除了需要合并两个相邻的数值以外，还需要根据各自的位置进行相应的计算。

比如当第一四分位数的位置上并不是特定的一个数值时，则取前一个数乘以 $\frac{1}{4}$ 的乘积加上后一个数乘以 $\frac{3}{4}$ 的乘积，而并不是两者的平均数，因为这样才更符合第一四分位数的定义。第三四分位数同理。
...
![boxplot-chart-1](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/8/14/1653767273e14024~tplv-t2oaga2asx-image.image)
...
![boxplot-chart-2](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/8/14/1653767273f4ff86~tplv-t2oaga2asx-image.image)
...
![boxplot-chart-3](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/8/14/1653767273fda531~tplv-t2oaga2asx-image.image)
