...
数学期望值指的是在概率论中，一个数值集合总体中各种**可能性**的结合。举一个“栗子”，一个袋子中装有若干来自我国北方的板栗，以及若干来自我国南方的锥栗。那么经过**无限次**取出、记录并放回之后，我们可以假设计板栗为 `-1`，计锥栗为 `1`，经过简单的统计计算得出取出样本中板栗的概率 $P_{\alpha}$ 为 $\frac{3}{5}$，而锥栗的概率 $P_\beta$ 为 $\frac{2}{5}$。

| 值 | 板栗（`-1`） | 锥栗（`1`） |
| :--- | :--- | :--- |
| 概率 $P$ | $\frac{3}{5}$ | $\frac{2}{5}$ |

根据数学期望的计算公式可得，该袋子中栗子的期望值为 $\text{-}\frac{1}{5}$。
...
![Map-Reduce-WordCount](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/4/7/162a021b2697a82d~tplv-t2oaga2asx-image.image)
