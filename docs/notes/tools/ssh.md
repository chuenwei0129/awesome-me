...
`ssh-copy-id` 是一个专门用于分发公钥的工具，它会自动处理：
- 创建 `~/.ssh` 目录（如果不存在）
- 设置正确的文件权限
- 将公钥追加到 `authorized_keys`（不会覆盖已有公钥）
...
按提示输入密码后，公钥就会被自动安装到服务器上。

#### 指定私钥路径

如果你有多把密钥，默认使用 `~/.ssh/id_rsa` 或 `~/.ssh/id_ed25519`，也可以显式指定：

```bash
ssh-copy-id -i ~/.ssh/id_ed25519_work user@server_ip
```

`-i` 参数指定要使用的公钥文件（私钥路径去掉 `.pub` 后缀即可）。

#### 多公钥支持

`ssh-copy-id` 的一个重要特性是**追加而非覆盖**：

- 第一次运行：会创建 `~/.ssh/authorized_keys` 文件并写入公钥
- 后续运行：会在文件末尾追加新公钥，不会删除已有的公钥

这意味你可以在同一台服务器上添加多台机器的公钥，或者同一台机器的多把公钥，多个用户都能独立访问。

#### 方法二：手动添加（备选）

如果服务器没有安装 `ssh-copy-id`，可以手动操作。
...
**注意**：`~/.ssh` 目录权限必须是 `700`，`authorized_keys` 文件权限必须是 `600`，否则 SSH 不认。
...
- `ssh-copy-id`：把公钥自动安装到远程服务器（推荐替代手动复制粘贴）
...
### A.3 `ssh-copy-id`：一键安装公钥到远程服务器

`ssh-copy-id` 是一个非常实用的工具，它可以自动把你的公钥安装到远程服务器的 `~/.ssh/authorized_keys` 文件中。
...
1. 要求你输入远程服务器的密码
2. 自动创建 `~/.ssh` 目录（如果不存在）
3. 自动设置正确的文件权限（`700` / `600`）
4. 把公钥追加到 `authorized_keys` 文件末尾
...
`ssh-copy-id` 的重要特性是**追加而非覆盖**：

- 第一次运行：创建 `~/.ssh/authorized_keys` 并写入公钥
- 后续运行：追加新公钥到文件末尾，不删除已有公钥

这使得你可以在同一台服务器上添加多台机器的公钥，或者同一台机器的多把公钥，实现多用户独立访问。

#### 与手动方法对比

| 方式              | 优点                                    | 缺点                          |
| ----------------- | --------------------------------------- | ----------------------------- |
| `ssh-copy-id`     | 自动处理权限、追加公钥、防止覆盖        | 需要服务器支持该命令          |
| 手动复制粘贴      | 任何环境都能用                          | 需要手动设置权限，容易出错    |

大部分 Linux/macOS 发行版都内置了 `ssh-copy-id`，推荐优先使用。

---

### A.4 `ssh-keygen`：生成 / 管理密钥对
