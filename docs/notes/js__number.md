...
JavaScript 的 Number 类型既神奇又复杂，能够处理各种数值运算，但也隐藏着一些不直观的行为。下面是一些典型的"反直觉"现象：
...
// ===== Demo 3：toFixed 方法的舍入问题 =====
console.log((1.005).toFixed(2)); // 返回的是 1.00 而不是期望的 1.01

// ===== Demo 4：大整数精度丢失导致无限循环 =====
function testPrecisionLoss() {
  let num = 2 ** 53;
  // 超过安全整数范围后，num++ 可能不会改变 num 的值
  // 导致循环条件永远无法满足
  while (true) {
    if (++num % 13 === 0) {
      return num;
...
// ===== Demo 5：JSON 解析时的大整数精度丢失 =====
JSON.parse('{"a":180143985094813214124}'); // 实际结果：{a: 180143985094813220000}

// ===== Demo 6：位运算的数值范围限制 =====
const a = 2 ** 31;
// 1. 值相等，但判等失败（因为位运算会转为 32 位有符号整数）
...
要理解这些现象，需要掌握一些[前置知识](#前置知识)。

### Number 定义

> 在 JavaScript 代码中，像 37 这样的数字字面量是浮点数值，而不是整数。在常见的日常使用中，JavaScript 没有单独的整数类型。
...
```js
// 乘法异常
console.log(18.9 * 100); // js => 1889.9999999999998，数学期望 => 1890
console.log(64.68 * 100); // js => 6468.000000000001，数学期望 => 6468
```

首先，18.9 这个数字转换成 IEEE 754 值如下图：

![18.9 的 IEEE 754 表示](https://raw.githubusercontent.com/chuenwei0129/my-picgo-repo/master/me/20240321114229.png)

尾数部分是无限循环的，计算机在存储时必然溢出，所以需要舍弃后面的数字。按[舍入规则](#舍入规则)计算，向下舍入，所以计算机中存储的值比实际数学中的值小，再通过乘法放大，误差也就显现出来了。
...
...1000000... (中间值)
// 实际数值小于中间值，向下舍入
// 所以实际存储时，从第 53 位开始的值都被省略
// 因此 18.9 的存储值小于真实值
```

那 64.68 经过计算后为什么反而大了呢？

```js
// 实际上 64.68 尾数部分的二进制是 10101110000101000111（无限循环）
// 舍入规则分析
000000 10101110000101000111 10101110000101000111 101011 (实际数值前 52 位)
...10000101000111... (实际数值从第 53 位起)
...10000000000000... (中间值)

// 等于中间值，且当前最低有效位（第 52 位）为 1（奇数）
// 根据"向偶数舍入"规则，向上舍入
// 存储的值为：
// 000000 10101110000101000111 10101110000101000111 101100
```

对比发现，存储时实际值后三位 `011` 向上舍入变成了 `100`，所以计算机中存储的值比实际数学中的值大。

验证一下：

![验证结果](https://raw.githubusercontent.com/chuenwei0129/my-picgo-repo/master/me/20240321133705.png)
...
> **计算误差并不是简单因为参与计算的项不能精确表示，其本质原因是有效位位数的限制。并不是所有参与计算的数值都是精确的，结果就一定是精确的。**

下面通过图示分析这个问题：

![0.1 的 IEEE 754 表示](https://raw.githubusercontent.com/chuenwei0129/my-picgo-repo/master/me/20240321140755.png)

![0.2 的 IEEE 754 表示](https://raw.githubusercontent.com/chuenwei0129/my-picgo-repo/master/me/20240321140952.png)

![0.3 的 IEEE 754 表示](https://raw.githubusercontent.com/chuenwei0129/my-picgo-repo/master/me/20240321141310.png)

![0.1 + 0.2 的计算过程](https://raw.githubusercontent.com/chuenwei0129/my-picgo-repo/master/me/20240321110221.png)
...
// 精度丢失的表现：
// 0.1 在 17 位精度时会丢失（计算机存储的值可能对应不止一个数学值）
0.10000000000000001 === 0.1; // true

// 总结：
// - 0.1 无法精确表达数学值，丢失了精度
// - (0.1).toPrecision(17) 无法一一对应数学值，也丢失了精度
// - 0.25 这样的数可以精确表达（因为 0.25 = 1/4 = 2^-2）
// - 我们期望的是数学值的结果，但计算机运算的是浮点数
```
...
![1.005 的 IEEE 754 表示](https://raw.githubusercontent.com/chuenwei0129/my-picgo-repo/master/me/20240321170809.png)

观察上图可以发现，1.005 在存储时明显向下舍入了，发生了精度丢失。按照存储值转换为十进制的结果是 `1.00499999999999989341858963598`，所以使用 `toFixed(2)` 时会得到 `1.00` 而不是期望的 `1.01`。

**解决方案**：对于需要精确舍入的场景，建议使用前面提到的[精度处理方案](#如何解决精度问题)。
...
JavaScript 中的常用按位操作符及描述如下表所示。**重要提示：所有的按位操作符的操作数都会被转成补码形式的有符号 32 位整数**。如果操作数的二进制表示超过 32 位，则只保留最后的 32 位进行计算。

> **性能提示**：现代 JS 引擎（如 V8）普遍使用 JIT 优化代码，位运算性能接近原生代码。因此在合适的场景下可以放心使用位运算，不仅不会影响性能，反而可能带来性能提升。
...
**小数点在数制中代表一种对齐方式**。比如说你要比较 `1000` 和 `200` 哪个比较大，你应该怎么做呢？你必须把它们**右对齐**：
...
然后发现 `1` 比 `0`（前面补零）大，所以 `1000` 比较大。那么如果是比较 `1000` 和 `200.01` 呢？这时候就不是右对齐了，而是**小数点对齐**：
...
**小数点位置在进制表示中是至关重要的，位置差一位整体就要差进制倍**（十进制就是十倍）。在计算机中也是这样，虽然计算机使用二进制，但在处理非整数时，也需要考虑小数点位置的问题，**无法对齐小数点就无法做加法、减法、比较这样的操作**。

在计算机中处理小数点位置有**浮点**和**定点**两种方式：

#### 定点数

**定点数就是小数点永远在固定的位置上**。比如说我们约定一种 32 位无符号定点数，它的小数点永远在第 5 位后面，这样：

- 最大能表示的数：`11111.111111111111111111111111111`
- 最小非零数：`2^-27`

**定点数实际上是提前对齐好的小数，而整数是一种特殊情况，小数点永远在最后一位之后。**

**优点**：实现简单，大部分运算和整数一样或略有变化

**缺点**：

1. **表示范围有限**：例如上面的例子中，最大只能表示 `31`（整数部分只有 5 位）
2. **精度不均匀**：表示很小的数时，大部分位都是 `0`，精度很差，不能充分利用存储单元

#### 浮点数

浮点数是为了克服定点数的缺点而设计的。它相当于**一个定点数加上一个阶码**，**阶码表示将这个定点数的小数点移动若干位**。**由于可以用阶码移动小数点，因此称为浮点数。**

在计算机科学中，浮点数是一种对实数的近似值数值表示法，由一个有效数字（即尾数）加上幂数来表示，通常是乘以某个基数的整数次指数得到。以这种表示法表示的数值，称为浮点数。

**简单理解**：浮点数是十进制科学记数法在计算机中的二进制表示，即**二进制的科学记数法**。
...
![32 位浮点数的存储结构](https://raw.githubusercontent.com/chuenwei0129/my-picgo-repo/master/js/v2-32e425cdb908c47586267e59228fcd22_1440w.png)

这 32 个二进制位的内存编号从高到低（从 31 到 0），共包含如下 3 个部分：

- **sign**：符号位（蓝色方块），1 位，表示正负
- **biased exponent**：偏移后的指数位（绿色方块），8 位，表示数的范围
- **fraction**：尾数位（红色方块），23 位，表示数的精度

#### 存储步骤详解

**步骤 1：将十进制数值转换为二进制数值**

![十进制转二进制示例](https://raw.githubusercontent.com/chuenwei0129/my-picgo-repo/master/computer/SCR-20220408-acz.png)

```text
遇到小数转换时，需要把整数和小数两部分分别进行处理：

【整数部分】整数 20 除以 2，直到商是 0 为止，取每次除 2 得到的余数结果
20 / 2 = 10  余数 0
10 / 2 = 5   余数 0
5  / 2 = 2   余数 1
2  / 2 = 1   余数 0
1  / 2 = 0   余数 1
→ 结果为得到的余数从下往上排列：10100

【小数部分】小数 0.25 乘以 2，直到不存在小数位为止，记下每次乘后的整数位结果
0.25 * 2 = 0.5  整数位 0
0.5  * 2 = 1.0  整数位 1
→ 结果为得到的整数位从上往下排列：01
...
二进制表示采用原码表示。`10100.01` 用科学记数法表示为：
...
将科学记数法的指数值加上偏移值，再转换为二进制：

- 单精度（32 位）偏移值：`127`
- 双精度（64 位）偏移值：`1023`

本例中：指数值为 `4`，加上偏移值后为 `131`，转换为 8 位二进制为 `1000 0011`

**步骤 5：计算尾数（小数部分）**

由于尾数只存储有效数字的**小数部分**，最高位的 `1` 规定不显式存储（隐式存在），计算或恢复数值时再把这个 `1` 补上。

科学记数法中的有效数字为 `1.010001`，因此尾数为 `010001`

如果尾数不足 23 位，则右边用 `0` 填充至 23 位：`0100 0100 0000 0000 0000 000`
...
符号位：0
指数位：10000011
尾数位：01000100000000000000000
```

完整的 32 位表示：`0 1000 0011 0100 0100 0000 0000 0000 000`

这里有一个可以[在线浮点数转二进制](https://tooltt.com/floatconverter/)的网站，我们来验证一下：

![验证结果](https://raw.githubusercontent.com/chuenwei0129/my-picgo-repo/master/me/20240307200242.png)

#### 反向推导验证

为了加深理解，我们再反向推导一遍，将浮点数 `0 1000 0011 0100 0100 0000 0000 0000 000` 恢复为十进制数：

1. **符号位**为 `0`，说明该数为正数
2. **阶码**为 `1000 0011`，转换为十进制是 `131`，因此实际指数值为 `131 - 127 = 4`
3. **尾数** = 小数部分 `0100 0100 0000 0000 0000 000` + 隐含值 `1` = `1.010001`
4. 因此该浮点数所表示的实际数值为 $1.010001 \times 2^4$
5. 小数点向右移动 4 位，得到二进制数 `10100.01`
6. 转换为十进制就是 `20.25`
...
了解了精度问题的原因后，我们需要知道如何在实际开发中处理这些问题。

### 方案一：容差比较（推荐用于简单场景）

对于浮点数比较，不要使用 `===`，而是判断两个数的差值是否小于一个很小的值（`Number.EPSILON`）：
...
对于金额等小数计算，可以先转换为整数（如"分"），计算完成后再转换回小数：
...
console.log(addMoney(18.9, 0)); // 18.9
```

### 方案三：使用第三方精度库（推荐用于复杂场景）

对于复杂的数值计算，建议使用成熟的第三方库：

**1. decimal.js**（推荐用于高精度计算）
...
### 方案四：使用 toFixed() 或 toPrecision() 格式化结果

对于展示用途，可以使用 `toFixed()` 或 `toPrecision()` 格式化结果：

```js
const result = 0.1 + 0.2;
console.log(result.toFixed(2)); // '0.30'
console.log(parseFloat(result.toFixed(10))); // 0.3
```

**注意**：`toFixed()` 本身也可能有舍入问题（如 Demo 3 所示），使用时需谨慎。

### 方案五：使用 BigInt 处理大整数

对于超出安全整数范围的大整数运算，使用 `BigInt` 类型：

```js
// 错误的做法
const bigNum = 9007199254740993; // 超出 Number.MAX_SAFE_INTEGER
console.log(bigNum); // 9007199254740992 (精度丢失！)

// 正确的做法
const bigNum = 9007199254740993n; // 使用 BigInt
...
// 使用字符串形式传输
const safeJsonStr = '{"id": "9007199254740993"}';
const obj = JSON.parse(safeJsonStr);
console.log(BigInt(obj.id)); // 9007199254740993n
```

### 最佳实践建议

1. **金融计算**：使用整数运算或 decimal.js
2. **简单的浮点数比较**：使用容差比较
3. **大整数运算**：使用 BigInt
4. **显示格式化**：使用 toFixed() 或 toPrecision()
5. **避免直接比较浮点数**：永远不要使用 `===` 比较浮点数计算结果
