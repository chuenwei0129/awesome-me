...
2. **无需分号**：在 **Sass** 的初始语法中，我们不需要在每个声明的末尾添加分号，**Sass** 会根据换行符来确定声明的结束。

#### 为什么使用 .scss 后缀？

虽然原始 Sass 语法可以让我们少写几个字符，但突如其来的改变让人一时很难接受。因此 **Sass** 开发者们推出了**类 CSS** 语法格式，也就是 **SCSS** 语法格式。
...
了解编译器的发展历程对以后的项目是有一定帮助的，因为有些较老的项目还是采用早期编译器进行开发的，会时常出一些问题。明白了这些发展过程才能知道出现问题时该如何查阅资料并解决问题。

**主要发展过程**：

1. **Ruby Sass**：最初的 **Sass** 编译器是使用 **Ruby** 语言开发的，它是第一个版本的 **Sass** 编译器。这个版本使用了一种全新的语法，引入了许多新的特性和概念，如嵌套规则、变量和混合器等。这个版本的 **Sass** 编译器由 **Ruby** 实现，需要通过 **Ruby** 的命令行接口或其他工具来编译 **Sass** 文件。
...
2. **LibSass** 和 **Node-sass**：随着 **Sass** 的流行，为了提高 **Sass** 编译的性能和扩展性，**Sass** 团队开发了 **LibSass** 和基于 **LibSass** 的 **Node-sass** 编译器。**LibSass** 是一个用 `C/C++` 编写的 **Sass** 编译器库，可以在多个平台和编程语言中使用。**Node-sass** 是一个使用 **LibSass** 的 **Node.js** 二进制绑定库，它通过将 **Sass** 文件编译为 **CSS**，提供了更快速和高效的 **Sass** 编译能力。
   ...
   > ⚠️ **Node-sass 的痛点**：如果你是早期的 **Sass** 使用者，那么你一定使用过 **Node-sass**，它给开发者带来的体验真是让人非常……其原因是 **Node** 版本和 **Node-sass** 版本对应不起来，有的时候我们升级了 **Node**，项目却运行不起来了；有的时候同事的电脑能运行起项目，自己的电脑却运行不了，其本质原因是两者的版本不对应。

3. **Dart Sass**（推荐）：为了提供更出色的性能和更好的兼容性，**Sass** 团队决定基于 **Dart** 语言重新实现 **Sass** 编译器。**Dart Sass** 是一个用 **Dart** 语言编写的全新 **Sass** 编译器，它通过 **JavaScript** 实现的 **Dart** 运行时运行。**Dart Sass** 保持了与 **Ruby Sass** 和 **LibSass** 兼容的语法和功能，但在性能和准确性方面有所提升。

   > ✅ 目前我们使用的是 **Dart Sass** 版本，体验还是非常好的。

## 二、为什么

### 1. CSS 的局限性

原生 CSS 虽然强大，但在大型项目中存在一些明显的局限性：

- **缺少变量机制**：颜色、字体大小等重复值需要手动复制粘贴，修改时容易遗漏
- **无法嵌套规则**：选择器的层级关系不够直观，代码结构难以反映 HTML 结构
- **缺少代码复用**：相似的样式块需要重复编写
- **没有计算能力**：无法进行数学运算和逻辑判断
- **难以模块化**：大型项目中样式文件难以组织和管理
...
- **更好的组织结构**：嵌套规则清晰地反映 HTML 结构
- **更少的代码重复**：变量、混合器、继承让代码更加 DRY（Don't Repeat Yourself）
- **更清晰的语法**：向后兼容 CSS，学习成本低
- **更强的可维护性**：修改主题色只需更改一个变量
- **更高的开发效率**：编程能力让复杂样式的生成更加简单
...
在上面的示例中，开始时 `$primary-color` 的值是 `red`，`.container` 使用这个值，背景色为 `red`。在后面的代码中，变量被更新为 `blue`，`.another-element` 使用更新后的值，背景色为 `blue`。

> **注意**：变量的更改不会影响已经使用过该变量的样式规则。`.container` 的背景色仍然是 `red`，只有在变量更改后使用的 `.another-element` 才会使用新值 `blue`。
...
- **官方文档**：[https://sass-lang.com](https://sass-lang.com)
- **在线编译器**：[https://www.sassmeister.com](https://www.sassmeister.com)
- **迁移指南**：从 @import 迁移到 @use - [官方迁移指南](https://sass-lang.com/documentation/at-rules/use)

---

**记住**：SCSS 是 CSS 的超集，所有有效的 CSS 也是有效的 SCSS。从简单开始，逐步引入高级特性，让代码更加优雅和易维护。
