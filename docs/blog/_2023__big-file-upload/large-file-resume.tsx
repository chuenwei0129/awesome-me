...
const FileUpload: React.FC = () => {
  ...
  const handleUpload = async (isResume = false) => {
    if (!file) return;
    ...
    try {
      ...
      let chunksToUpload = fileChunks;
      if (isResume) {
        const uploadedChunksIndices = data.uploadedChunks.map((chunk: string) => parseInt(chunk.split('-').pop()!, 10));
        setUploadedChunks(uploadedChunksIndices);
        chunksToUpload = fileChunks.filter((_, index) => !uploadedChunksIndices.includes(index));
      }

      const requests = chunksToUpload.map((chunk) => {
        const chunkIndex = fileChunks.indexOf(chunk);
          ...
          axios.post('http://localhost:3333/upload/chunk', formData, {
            onUploadProgress: (progressEvent) => {
              ...
              setChunkProgress((prevProgress) => {
                ...
                newProgress[chunkIndex] = percentCompleted;
                return newProgress;
              });
            },
  ...
  return (
    <div className="p-6 max-w-lg mx-auto bg-white rounded-xl shadow-md space-y-6">
      ...
      <Button onClick={() => handleUpload(false)} type="primary" block disabled={isUploading}>
        上传文件
      </Button>
      <Button onClick={handlePause} type="default" block disabled={!isUploading}>
        暂停上传
      </Button>
      <Button onClick={handleResume} type="default" block disabled={isUploading}>
        继续上传
      </Button>
      <div className="mt-6">
        {chunkProgress.map((percent, index) => {
          const isUploaded = uploadedChunks.includes(index);
          return <Progress key={index} percent={isUploaded ? 100 : percent} status="active" className="mt-2" />;
        })}
      </div>
    </div>
  );
